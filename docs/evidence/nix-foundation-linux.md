...
the primary Linux builder's store:

```console
$ direnv exec . nix build --store ssh-ng://mjc@PRIMARY_LINUX_BUILDER \
...
The primary Linux builder ran Linux 6.18.44 on x86_64 with Nix 2.35.2. Executing the package there
...
The package executed successfully on both Linux builders. The current scaffold
...
`narjar-deps-0.1.0.drv` on the independent Linux builder rebuilt Cargo checks,
the release binary, and the no-run test binary successfully, but Nix reported
that the compressed `target.tar.zst` output differed from the primary builder's
copy.
