...
- `scripts/casync-zfs-experiment` for a matched flat-versus-chunked storage
  ...
  requires matching properties including `compression=zstd` by default
  (`zstd` is the OpenZFS alias for `zstd-3`); pass `--compression zstd-N`
  for a controlled compression-level diagnostic,
  and accepts
