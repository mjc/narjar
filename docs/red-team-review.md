...
Resolved by scope. Narjar accepts raw bytes, `.nar.zst`, and `.nar.xz`, and only
streams the matching decoder to validate the raw hash and size; it does not
recompress or semantically parse a NAR. Hashing/counting is O(1) memory and
decompressed output is capped. "Validated" means route, size, hash, metadata,
